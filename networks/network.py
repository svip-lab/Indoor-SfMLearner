...
from .particalconv import PartialConv2d
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[], dropout=False, dropout_p=0.5):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, dropout=dropout, dropout_p=dropout_p)
    elif netD == 'n_layers':  # more options
        net = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, dropout=dropout, dropout_p=dropout_p)
    elif netD == 'pixel':     # classify if each pixel is real or fake
        net = PixelDiscriminator(input_nc, ndf, norm_layer=norm_layer, dropout=dropout, dropout_p=dropout_p)
    elif netD == 'partial':
        net = NLayerDiscriminatorPartial(input_nc, ndf, n_layers_D, norm_layer=norm_layer, dropout=dropout,
                                         dropout_p=dropout_p)
    else:
...
class NLayerDiscriminatorPartial(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, dropout=False, dropout_p=0.5):
        ...
        n = n_layers
        self.convs["conv_{}".format(n)] = PartialConv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1,
                                                        padding=padw, bias=use_bias, return_mask=True)
        self.convs["norm_{}".format(n)] = norm_layer(ndf * nf_mult)
        self.convs["relu_{}".format(n)] = nn.LeakyReLU(0.2, False)
        if dropout:
            self.convs["dropout_{}".format(n)] = nn.Dropout(dropout_p, True)
